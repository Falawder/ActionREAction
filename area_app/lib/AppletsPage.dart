...
class Applets extends StatefulWidget {
  _AppletsPageState createState() => new _AppletsPageState();
}

class _AppletsPageState extends State<Applets> {
  bool isFill = false;
  List<List> AllData = [];
  ...
  LoadData() async {
    ...
    var tmp = [];
    list.forEach((n) =>
       tmp.add([n['actiontitle'], n['reactiontitle'], n['description'], n['isOn'], n['action'], n['reaction']])
    );
  ...
  Widget createCards(){
    List<Widget> cards = new List.generate(AllData.length, (i)=>new CustomCard1(i, AllData));
...
class CustomCard1 extends StatelessWidget{
  ...
  const CustomCard1(this.i, this.list);
  ...
  Widget tile(i, list, context){
    ...
    child = GestureDetector(
      onTap: () async {
        final response = await Post.acarParser(i, list);
